...
const App = () => {
  ...
  const requestPermissions = async () => {
    try {
      ...
      if (PermissionsAndroid.PERMISSIONS.READ_MEDIA_IMAGES) {
        storagePermissions.push(PermissionsAndroid.PERMISSIONS.READ_MEDIA_IMAGES);
      }
      if (PermissionsAndroid.PERMISSIONS.READ_MEDIA_VIDEO) {
        storagePermissions.push(PermissionsAndroid.PERMISSIONS.READ_MEDIA_VIDEO);
      }
      ...
      const results = await Promise.all(
        storagePermissions.map(permission =>
          PermissionsAndroid.request(permission, {
            ...
            message: 'App needs access to your storage to select and scan files.',
            buttonPositive: 'OK',
          })
        )
      );
  ...
  const handleFolderScan = async (files, folderPath) => {
    ...
    try {
      ...
      Alert.alert('Success', `${files.length} files from folder processed and indexed successfully`);
    } catch (error) {
